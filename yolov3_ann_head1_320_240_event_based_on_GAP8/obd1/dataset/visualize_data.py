...
def dump_image_with_labels(event_cube,ann_array,target_spatial_size,output_path,index,create_histo_frames = True):
    ...
    if create_histo_frames:
        ...
        num_channels = event_cube.shape[1]
        ...
        dvs_frame = make_dvs_frame(events_only_tensor.numpy(), height=target_spatial_size[0],width=target_spatial_size[1],color=False, clip=3,forDisplay = True)
    ...
    img_full_name = save_images_for_matchscore_calculation(output_path,file_id = index,img_array = dvs_frame,isImgFrame = False,target_size=(target_spatial_size[1],target_spatial_size[0]))
