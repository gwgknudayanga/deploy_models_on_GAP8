...
class YOLOBase(torch.nn.Module):
    ...
    def yolo_raw(self, x: torch.tensor) -> torch.tensor:
        ...
        return x.reshape(N,
                         self.num_anchors,
                         -1, H, W).permute(0, 1, 3, 4, 2) # now (batch,anchors,h,w,-1,T)

    def init_model(self, input_dim: Tuple[int, int] = (448, 448), channels = 4) -> None:
        ...
        H, W = input_dim
        N, C, T = 1, channels, 1
        input = torch.rand(N,C, H, W).to(self.anchors.device)
...
class YOLOtarget:
    ...
    def collate_fn(self,batch: List[Tuple[torch.tensor,torch.tensor]])-> Tuple[torch.tensor,
                              List[torch.tensor], List[List[torch.tensor]]]:
        
        images, targets, bboxes = [], [], []

        for event_cube,annotation in batch:

            images += [event_cube]
            tgt = [self.forward(ann_tensor) for ann_tensor in annotation]
            bboxes += [annotation]
