...
class ste_sigmoid(torch.autograd.Function):
    ...
    def backward(ctx,grad_output):
        ...
        #print(grad_output * sigmoid_x.backward())
        return grad_output*torch.sigmoid(x)*(1-torch.sigmoid(x))

class STE_sigmoid(nn.Module):

    def __init__(self,alpha: float = 1.,beta: float = 2.):
        super(STE_sigmoid, self).__init__()

    def forward(self,x):
        return ste_sigmoid.apply(x)

class CustomLIFNode(neuron.LIFNode):
    
    def __init__(self,decay_input = False, surrogate_function = None,
                 detach_reset = False, v_threshold = 1.0):
        super().__init__(surrogate_function = surrogate_function,detach_reset = detach_reset,decay_input = decay_input, v_threshold = v_threshold)

    def single_step_forward(self, x: torch.Tensor):
        #print("within single step forwarddddddddddddddddddddddddddddddddddddd ")
...
class Network(YOLOBase):
    ...
    def __init__(self,
        ...
        self.backend_blocks = torch.nn.ModuleList([
            ...
            nn.Conv2d(in_channels, 16, kernel_size=3, padding=1, stride=1, bias=False), #2
            nn.BatchNorm2d(16),
            nn.MaxPool2d(2,2),
            nn.ReLU(),
            nn.Dropout(0.2),

            nn.Conv2d(16,32, kernel_size=3, padding=1,stride=1, bias=False), #2
            nn.BatchNorm2d(32),
            nn.MaxPool2d(2,2),
            nn.ReLU(),
            nn.Dropout(0.2),
