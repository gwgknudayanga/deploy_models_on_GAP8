...
import cv2
from .input_transforms import augment,letterbox,vflip,hflip,rotate90antclk,rotate90clk,random_affine
...
class evCIVIL(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        if self.frame_based:
            ...
            if self.augment:
                if random.random() > 0.6:
                    ...
                    #print("eeeeeeeeeeeeeeeeeeeee ",event_cube.shape)
        ...
        if self.augment:
            if random.random() < 0.6:
                event_cube,ann_array = vflip(event_cube,ann_array,self.target_spatial_size - 1)
            if random.random() < 0.5:
                event_cube,ann_array = hflip(event_cube,ann_array,self.target_spatial_size - 1)
            if random.random() < 0.5:
