...
def draw_labels_on_image(image_path,ann_array,format = "yolo",original_path = "."):
    ...
    if image is None:
        print("no image forrrrrrrrrrrrrr ",original_path)
        return
...
def dump_image_with_labels(event_cube,ann_array,target_spatial_size = 448,original_path = "."):

    desired_ann = ann_array[:, [0,1,2,3,5]]
    desired_ann[:,[0,1,2,3,4]] = desired_ann[:,[4,0,1,2,3]]

    first_t_bin = get_events_only_2D_tensor(event_cube[:,[0,1],:,:])
    second_t_bin = get_events_only_2D_tensor(event_cube[:,[2,3],:,:])
    ...
    events_only_tensor[:,[0,1,2,3]] = events_only_tensor[:,[0,3,2,1]]
    dvs_frame = make_dvs_frame(events_only_tensor.numpy(), height=target_spatial_size,width=target_spatial_size,color=False, clip=3,forDisplay = True)
    print("dvs shapeeeeeeeeeeeeeeeeeee ",dvs_frame.shape,"   ",event_cube.shape)

    img_full_name = save_images_for_matchscore_calculation(".",file_id = 2,img_array = dvs_frame,isImgFrame = False,target_size=(target_spatial_size,target_spatial_size))
    
    if img_full_name:
        draw_labels_on_image(img_full_name,desired_ann,format = "yolo",original_path = original_path)
